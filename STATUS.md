...
| County-wide land-use authority bridge | Issue #428 makes the exception to the county-rule guard explicit and auditable: Metis applies county land-use rules only for a current, individually reviewed `COUNTY_WIDE` claim with copied evidence and a source that is still verified as the local official authority. Incorporated, unincorporated-only, unknown, review-due/stale, revoked-source, and boundary-unresolved scopes all remain conditional until parcel-boundary authority resolution exists. |
