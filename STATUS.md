...
| Jurisdiction claim/evidence ledger | Issue #310 makes every new profile write an atomic claim + copied-evidence + read-projection operation. Source authority validation is persisted separately from expected authority, stale candidate/source review races fail closed, superseded claims remain traceable, and legacy JSON is visibly labeled as lacking provenance. Authority verification UI/operations, snapshot archival, researched legacy migration, freshness/contradictions, and coverage workflows remain in #296. |
| Source authority review workflow | Issue #312 adds a filtered super-admin source queue and append-only verify/reject/reset decisions with authority evidence, server-owned reviewer/time, exact source-version concurrency, and atomic current-state projection. Discovery/AI cannot verify sources and existing claims are never retroactively promoted. Bulk/adapted authority registries, snapshot archival, claim re-review, freshness/contradictions, migration, and coverage workflows remain in #296. |
