...
| Jurisdiction claim/evidence ledger | Issue #310 makes every new profile write an atomic claim + copied-evidence + read-projection operation. Source authority validation is persisted separately from expected authority, stale candidate/source review races fail closed, superseded claims remain traceable, and legacy JSON is visibly labeled as lacking provenance. Authority verification operations shipped in #312 and snapshot archival in #314; researched legacy migration, freshness/contradictions, and coverage workflows remain in #296. |
| Source authority review workflow | Issue #312 adds a filtered super-admin source queue and append-only verify/reject/reset decisions with authority evidence, server-owned reviewer/time, exact source-version concurrency, and atomic current-state projection. Discovery/AI cannot verify sources and existing claims are never retroactively promoted. Snapshot archival shipped in #314; bulk/adapted authority registries, claim re-review, freshness/contradictions, migration, and coverage workflows remain in #296. |
| Immutable jurisdiction evidence | Issue #314 archives the exact Jina Markdown reviewed by AI under non-overwriting SHA-256 R2 keys and records every retrieval, including unchanged content. New AI candidates and claims retain server-trusted snapshot provenance; legacy snapshot-less AI candidates fail closed. This is a transformed evidence representation, not mislabeled original county source bytes. Freshness/re-review, contradictions, researched migration, and coverage workflows remain in #296. |
...
1. Continue the national jurisdiction intelligence/on-demand research architecture: archive evidence snapshots, implement freshness/contradiction and claim re-review, research/migrate legacy fields, then add coverage/on-demand workflows and Volusia as one canonical proof case.
