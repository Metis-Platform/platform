...
| Field-report audit privacy (#488) | Merged: jurisdiction field-report audit events retain only tenant/user/request correlation and jurisdiction identity. Submitted strategy, field, label, and free-text reason are excluded from the audit ledger; route coverage proves the privacy boundary. |
...
| Migration connectivity resilience (#506) | In progress: after the #505 migration workflow hit a transient Neon P1001 and succeeded unchanged on retry, the main-only deploy is being hardened to retry connectivity failures only. |
...
| Document upload and trace hardening | PR #474 requires a tenant-owned deal before issuing a document upload capability, preventing cross-deal orphan uploads. Document creation/deletion now write tenant-scoped, request-correlated audit events containing only document/deal identity, never filename, key, MIME type, size, type, or content. |
| Task-comment trace coverage | PR #477 records each task comment atomically with a tenant-scoped, request-correlated audit event containing only task/comment identity, never comment content or user PII. Foreign tasks create neither a comment nor audit evidence; the exact Task Board journey is catalog-enforced and awaits isolated QA execution. |
| Wholesale buyer-profile trace coverage | PR #479 records buyer-profile creation and update atomically with tenant-scoped, request-correlated audit events containing only buyer contact identity, never PII, investment criteria, active state, or request payload. Invalid or foreign saves write neither profile/contact records nor audit evidence; the two Buyer workflow journeys await isolated QA execution. |
| Wholesale buyer-link isolation and trace coverage | PR #481 requires a same-tenant BUYER contact before linking it to a Wholesale deal, preventing cross-tenant contact attachment. Link/unlink operations atomically record request-correlated events containing only deal/contact identity; foreign buyer links write neither the relation nor audit evidence. |
