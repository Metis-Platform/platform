...
| Production dependency hardening | Issue #338 identified 11 production audit findings. PR #339 moves the Prisma CLI to build tooling and resolves PostCSS to 8.5.19, reducing the production-only audit to five high findings in the `react-simple-maps` D3 chain; that map dependency needs a dedicated compatibility remediation. |
