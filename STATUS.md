...
| Development portability verification | PR #288 shipped a platform-neutral bootstrap, context/env checks, and dev-container contract. Issue #295 is open because its macOS and approved remote-Linux acceptance runs have not yet been evidenced; each host must use its own revocable non-production secrets and must not mutate shared integration or production. |
