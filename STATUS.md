...
| Spreadsheet import containment | Issue #290 removes the vulnerable XLS/XLSX parser and retains only bounded CSV input. The follow-up aligns the file picker with that contract, neutralizes formula-like values in downloaded error CSVs, and rejects oversized declared multipart payloads before form parsing. Import audit/correlation metadata and a route-level boundary test remain open. |
