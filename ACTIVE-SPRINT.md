...
- [ ] **Mutation observability** — #341 establishes request-to-audit correlation for parcel/jurisdiction routes and now every existing server action that uses `emitAuditEvent`, including `DEAL_CREATED`; remaining: tenant-safe operator retrieval and QA trace correlation.
