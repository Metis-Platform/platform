...
- [ ] **Spreadsheet import safety (#290)** — #327 removes the vulnerable `xlsx` parser and contains imports to bounded CSV; this follow-up aligns the upload UI, formula-safe error exports, and declared-payload rejection. Remaining: import audit/correlation metadata and a final route-level boundary test.
...
- [ ] **Mutation observability** — #341 establishes request-to-audit correlation for existing parcel and jurisdiction mutations; remaining: broader semantic event coverage, tenant-safe operator retrieval, and QA trace correlation.
